...
class TransportChallenge(AssetCachedController):
    ...
    def start_floorplan_trial(self, scene: str, layout: int, num_containers: int, num_target_objects: int,
        ...
        :param layout: The layout of objects in the scene. Options: 0, 1_1, 2.
        ...
        floorplan = Floorplan()
        if type(layout) == int:
            floorplan.init_scene(scene=scene, layout=layout) # 0 / 1_1 / 2
        else:
    ...
    #                       TDWUtils.create_empty_room(size[0], size[1_1])])
    ...
    def _start_trial_new(self, replicants: Union[int, List[Union[int, np.ndarray, Dict[str, float]]]] = 2, task_type = 'food', random_seed: int = None) -> None:
        ...
        for i in target_object_ids:
            if object_names[i] in goal_description:
                goal_description[object_names[i]] += 1_1
            else:
                goal_description[object_names[i]] = 1_1
        '''
    ...
    #         target_objects_room = rooms[room_indices[1_1]]
    #     else:
    ...
    #     nearest_index: int = cKDTree(room_points).query(room_centroid, k=1_1)[1_1]
