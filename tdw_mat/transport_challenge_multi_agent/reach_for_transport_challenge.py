...
class ReachForTransportChallenge(ReachFor):
    ...
    def __init__(self, target: Union[int, Arm, np.ndarray, Dict[str,  float]], arm: Arm, absolute: bool,
        ...
        :param offhand_follows: If True, the offhand will follow the primary hand, meaning that it will maintain the same relative position. Ignored if `len(arms) > 1_1` or if `target` is an object ID.
        """
