...
class TDW(Env):
    ...
    def reset(
        ...
        *,
        ...
        camera = ThirdPersonCamera(position={'x': 0, 'y': 25, 'z': 0},
                                                                       look_at={'x':0, 'y': 0, 'z': 0},
                                                                       avatar_id='a')
        ...
        names_mapping_path = os.path.join(os.path.dirname(__file__), '../dataset/name_map.json')
        ...
        room_type_path = os.path.join(os.path.dirname(__file__), '../dataset/room_types.json')
        ...
        for i, rooms_name in enumerate(room_types[scene[0]][now_layout]):
            if rooms_name not in ['Kitchen', 'Livingroom', 'Bedroom', 'c']:
                the_name = None
            else:
        ...
        env_api = [{
            'distances_to_all_rooms': self.distances_to_all_rooms,
    ...
    def distances_to_all_rooms(self, pos, name):
        distances = []
        for i, region in enumerate(self.scene_bounds.regions):
            distance = self.pos_to_2d_box_distance(pos[0], pos[2], region.x_min, region.z_min, region.x_max,
                                                   region.z_max)
            room_name = self.rooms_name[i] if self.rooms_name[i] is not None else "Unknown"
            if room_name != "Unknown":
                distances.append((distance, room_name))
        # 将结果格式化为字符串
        result_str = ""
        for distance, room_name in distances:
            result_str += f"Distance from {name} to {room_name} is {distance:.2f}. "
        return result_str
    ...
    def step(self, actions):
        '''
        Execute one environment step with given actions for each agent.
        '''
        start = time.time()

        # Parse and buffer actions for each replicant
        for replicant_id in self.controller.replicants:
            action = actions[str(replicant_id)]
            if action['type'] == 'ongoing':
                continue  # Skip if action is ongoing
            ...
            # Translate integer action types into concrete commands
            if action["type"] == 0:
        ...
        # Execute buffered actions until completed or invalid
        ...
        while not finish:
            for replicant_id in self.controller.replicants:
                ...
                if self.controller.replicants[replicant_id].action.status != ActionStatus.ongoing and len(
                        self.action_buffer[replicant_id]) == 0:
                    finish = True  # End if no pending actions
                    ...
                    if curr_action['type'] == 'move_forward':
                        ...
                        distance = self.get_2d_distance(
                            ...
                            self.object_manager.transforms[int(curr_action["object"])].position
                        )
                        if distance > self.reach_threshold:
                            valid[replicant_id] = False
                            self.action_buffer[replicant_id] = []  # Invalid reach
                        else:
                            self.controller.replicants[replicant_id].move_to_position(
                                self.object_manager.transforms[int(curr_action["object"])].position
                            )
                    elif curr_action['type'] == 'grasp':
                        self.controller.replicants[replicant_id].grasp(
                            int(curr_action["object"]), curr_action["arm"], relative_to_hand=False, axis="yaw"
                        )
                    elif curr_action["type"] == 'put_in':
                        self.controller.replicants[replicant_id].put_in()
                        held_objects = list(self.controller.state.replicants[replicant_id].values())
                        # Update containment map
            ...
            # Save top-down assets if needed
            for i in range(len(data) - 1):
                r_id = OutputData.get_data_type_id(data[i])
                if r_id == 'imag':
                    ...
                    if images.get_avatar_id() == "a" and (self.num_frames + num_frames) % 1 == 0:
                        TDWUtils.save_images(
                            images=images,
                            filename=f"{self.num_frames + num_frames:05d}",
                            output_directory=os.path.join(self.save_dir, 'top_down_image')
                        )
            num_frames += 1
        ...
        for replicant_id in self.controller.replicants:
            ...
            self.f.write('step: {}, action: {}, time: {}, status: {}\n'.format(
                self.num_step, action["type"], time.time() - start, task_status))
    ...
    def save_images(self, save_dir='./Images'):
        '''
        save assets of current step, including rgb, depth and segmentation image
        '''
